...
from tg_bot import dispatcher, OWNER_ID, SUDO_USERS, FED_LOGS, LOGGER
...
def user_join_fed(bot: Bot, update: Update, args: List[str]):
	...
	fed_id = sql.get_fed_id(chat.id)
	fed_nme = sql.get_fed_info(fed_id)
	...
	if is_user_fed_owner(fed_id, user.id):
		...
		if FED_LOGS:
			bot.send_message(FED_LOGS,
					f"{mention_html(user.id, user.first_name)} Successfully Promoted! in {fed_nme['fname']} Federation.", parse_mode=ParseMode.HTML)
		else:
...
def user_demote_fed(bot: Bot, update: Update, args: List[str]):
	...
	if is_user_fed_owner(fed_id, user.id):
		...
		if FED_LOGS:
			bot.send_message(FED_LOGS,
					f"{mention_html(user.id, user.first_name)} Successfully Demoted! in {fed_nme['fname']} Federation.", parse_mode=ParseMode.HTML)
		else:
			update.effective_message.reply_text("Set FED_LOGS (var) for Federation Logs.")
	else:
		update.effective_message.reply_text("Only federation owners can do this!")
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
	...
	if fban:
		...
		# Send message to FED_LOGS
		if FED_LOGS:
                        bot.send_message(FED_LOGS, "<b>#FBANNED</b>" \
							"\n<b>Federation:</b> {}" \
							"\n<b>Federation Admin:</b> {}" \
							"\n<b>User:</b> {}" \
							"\n<b>User ID:</b> <code>{}</code>" \
...
def unfban(bot: Bot, update: Update, args: List[str]):
	...
	# Send message to FED_LOGS
	if FED_LOGS:
		bot.send_message(FED_LOGS, "<b>#UN-FBANNED</b>" \
						 "\n<b>Federation:</b> {}" \
						 "\n<b>Federation Admin:</b> {}" \
						 "\n<b>User:</b> {}" \
						 "\n<b>User ID:</b> <code>{}</code>".format(info['fname'], mention_html(user.id, user.first_name), user_target, fban_user_id), parse_mode="HTML")
	else:
		lgs = "Set FED_LOGS (var) for Federation Logs\nThis message will be delete after 5 seconds."
		update.effective_message.reply_text(lgs)
		time.sleep(5)
		lgs.delete()					 
	# Send message to owner if fednotif is enabled
	if getfednotif:
